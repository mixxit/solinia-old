...
//
#include "mod_melee_success_effect.h"
...
//		update
//--------------------------------------------------------------
bool CModMeleeSuccessEffect::update(CTimerEvent * event, bool applyEffect)
{
	...
	player->meleeSuccessModifier( (sint32)(_Modifier1 + _Modifier2) );
...
//		removed
//--------------------------------------------------------------
void CModMeleeSuccessEffect::removed()
{
...
//		activate
//--------------------------------------------------------------
void CModMeleeSuccessEffect::activate()
{
	CCharacter *actor = PlayerManager.getChar(_CreatorEntityId);
	if (!actor)
	{
		nlwarning("<CModMeleeSuccessEffect::activate> Cannot find actor entity or not a player");
		return;
	}
	
	CModMeleeSuccessEffect *effect = new CModMeleeSuccessEffect(actor->getEntityRowId(), 
		getEndDate()+CTickEventHandler::getGameCycle(), 
		EFFECT_FAMILIES::PowerModMeleeSkill,
		_Modifier1,
		_Modifier2);
	
	if (!effect)
	{
		nlwarning("<CModMeleeSuccessEffect::activate> Failed to allocate new CModMeleeSuccessEffect");
		return;
	}
...
//-----------------------------------------------------------------------------
// Persistent data for CModMeleeSuccessEffect
//-----------------------------------------------------------------------------

#define PERSISTENT_TOKEN_FAMILY RyzomTokenFamily
#define PERSISTENT_CLASS CModMeleeSuccessEffect
